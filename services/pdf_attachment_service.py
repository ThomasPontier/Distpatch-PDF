...
class PDFAttachmentService:
    ...
    @staticmethod
    def create_stopover_attachment(pdf_path: str, stopover: Stopover, output_dir: str = None) -> Optional[str]:
        """
        ...
        try:
            ...
            with fitz.open(pdf_path) as source_doc:
                if 1 <= stopover.page_number <= len(source_doc):
                    ...
                    new_doc = fitz.open()
                    new_doc.insert_pdf(source_doc, from_page=stopover.page_number-1, to_page=stopover.page_number-1)
